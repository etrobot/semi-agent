...
def get_avatar(twitter_id, list_id):
    url = os.getenv("TURSO_DATABASE_URL")
    auth_token = os.getenv("TURSO_AUTH_TOKEN")
    conn = libsql.connect("notes.db", sync_url=url, auth_token=auth_token)
    result = conn.execute("SELECT imgSrc FROM avatars WHERE twitterId = ?", (twitter_id,))
    row = result.fetchone()
    if row:
        return row[0]
    else:
        fetch_avatar_data(list_id)
        result = conn.execute("SELECT imgSrc FROM avatars WHERE twitterId = ?", (twitter_id,))
        row = result.fetchone()
        return row[0]
...
def fetch_avatar_data(list_id):
    ...
    headers = {
        'Authorization': os.environ["NITTER_AUTH"]
    }
    results = []
    next_cursor = None
    while True:
...
def send2turso(entries_with_comments):
    """Send DataFrame entries to the Turso database."""
    data = []
    userId = "987654321"

    for item in entries_with_comments:
        entry = item['entry']
        data.append({
            "link": entry.link,
            "title": item['comment'],
            "content": entry.title,
            "css": get_avatar(entry.author),
            "userId": userId,
            "authorId": entry.author,
            "createdAt": entry.published_parsed
        })
    ...
    for index, row in df.iterrows():
        insert_sql = """
        INSERT INTO note (link, title, content, css, userId, authorId, createdAt)
        VALUES (?, ?, ?, ?, ?, ?, ?)
        """
        conn.execute(insert_sql, (
        row['link'], row['title'], row['content'], row['css'], row['userId'], row['authorId'], row['createdAt']))
